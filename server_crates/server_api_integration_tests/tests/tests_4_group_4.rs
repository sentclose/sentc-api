...
#[tokio::test]
async fn test_18_re_invite_group_owner()
{
	let secret_token = &APP_TEST_STATE.get().unwrap().read().await.secret_token;
	let group = &GROUP_TEST_STATE.get().unwrap().read().await[0];
	...
	let user_keys = group
		.decrypted_group_keys
		.get(user_to_invite.user_id.as_str())
		.unwrap();
	...
	let url = get_url("api/v1/group/".to_owned() + group.group_id.as_str() + "/re_invite/" + creator.user_id.as_str());
