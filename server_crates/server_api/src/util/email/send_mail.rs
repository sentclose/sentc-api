...
impl Email
{
	...
	pub async fn send_email_smpt(&self, to: &str, subject: &str, body_txt: String, body_html: String) -> Result<(), ServerCoreError>
	{
		...
		let to = to
			.parse()
			.map_err(|_e| ServerCoreError::new_msg(400, CoreErrorCodes::EmailMessage, "Error in email message"))?;
